...
public class CacheService {
	...
	private ShardedJedisPool getPool() {
		if (pool == null) {
			try {				
				...
				config.setMaxActive(getConfig().getInt(JEDIS_POOL_MAXACTIVE));
	...
	public String getEnable() {
		return getConfig().getString("jedis.enable", "1");
	}
	 ...
	 public static void main(String[] args) {  
			...
			byte[] notice= CacheService.getInstance().getBytes("notice".getBytes());
			if(notice !=null){
