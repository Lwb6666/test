	...
	<div id="myaccount">
		...
		<div class="wraper mac-box mt20">
            ...
            <div class="myacc-dqb fl">
                <p class="my-hqx"><em>年化收益</em><span class="f20"><fmt:formatNumber value="${fixTenderRealVo.apr}" pattern="##.##" />%</span></p>
                <p class="my-hqx"><em>已赚利息</em><span class="f20 red">0.00</span><strong class="gary2">元</strong></p>
                <p class="my-hqx"><em>加入金额</em><span><fmt:formatNumber value="${fixTenderRealVo.account}" pattern="#,##0.00" />元</span></p>
                <div class="clearfix"></div>
                <p class="my-hqx"><em>预计收益</em><span>${fixTenderRealVo.yqsyStr}元</span></p>
                <p class="my-hqx"><em>使用红包</em><span>${fixTenderRealVo.redMoney}元</span></p>
                <p class="my-hqx"><em>剩余天数</em><span><c:if test="${fixTenderRealVo.remainDay<0}"> 0天 </c:if> 
						<c:if  test="${fixTenderRealVo.remainDay>=0}">
                 			${fixTenderRealVo.remainDay}天
                 		</c:if></span></p>
                <div class="clearfix"></div>
                <p class="my-hqx"><em>满宝日期</em><span><fmt:formatDate value="${fixTenderRealVo.successTime}" pattern="yyyy-MM-dd" /></span></p>
                <p class="my-hqx"><em>到期日期</em><span><fmt:formatDate value="${fixTenderRealVo.lockEndTime}" pattern="yyyy-MM-dd" /></span></p>
            </div>
        
            <div class="myacc-right fl">
               <div class="zhuantai f24">
               		<c:if test="${fixTenderRealVo.status==0 }">收益中</c:if>
					<c:if test="${fixTenderRealVo.status==2 }">退出中</c:if>
               </div>
            </div>
        </div>
		</div>
		...
		<div class="wraper mt20">
			<div class="prduct-menu  background clearfix">
				<div class="menu-tbl">
					<ul class="col2">
						<li onclick="load_tb(1)" class="active">投标信息</li>
...
// 		processTabStyle(obj);
		...
		$.ajax({
			url : '${basePath}/dingqibao/tab1tbDetail/'+'${fixTenderRealVo.fixBorrowId}'+'/'+pageNo+'.html',
			data : dataParam,
			type : 'post',
