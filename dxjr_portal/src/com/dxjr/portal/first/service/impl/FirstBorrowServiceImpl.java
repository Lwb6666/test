 ...
 */
@Service
public class FirstBorrowServiceImpl implements FirstBorrowService {
	...
	public String saveTenderFirstBorrow(Integer userId, FirstBorrowOpenCnd firstBorrowOpenCnd) throws Exception {
		...
		// 计算投标的最大有效金额
		BigDecimal effectiveMoney = firstTenderDetailService.isEffectiveMoney(firstBorrowVo, userId, firstBorrowOpenCnd.getTenderMoney(), accountVo);
		// 验证投标直通车数据是否正确
		...
		if (firstBorrow.getStatus() == Constants.FIRST_BORROW_STATUS_SUCCESS_APPROVE_PASS) {
			...
			// 帐号冻结扣除，直通车总额增加
			packageAccountFirst(accountVo, effectiveMoney);
			// 保存直通车最终认购资金使用记录
			packageFirstTenderRealAccountLog(userId, request, accountVo, effectiveMoney, firstBorrow);
			// 保存最终开通记录
			FirstTenderReal firstTenderReal = packageFirstTenderReal(userId, request, accountVo, effectiveMoney, firstBorrow);
			// 更新开通记录状态
			...
			if (firstBorrow.getPlanAccount().intValue() == firstBorrow.getRealAccount().intValue()) {
				...
				firstBorrowAppr.setRemark("该期直通车上车已满!");
				firstBorrowAppr.setStyle(Constants.APPROVE_STYLE_UP_OR_DOWN);
	...
	private void packageFirstAccountLog(Integer userId, HttpServletRequest request, AccountVo accountVo, BigDecimal effectiveMoney, FirstBorrow firstBorrow) throws Exception {
		...
		firstLogRemark.append("开通投标直通车成功，资金冻结成功。投标直通车：");
		firstLogRemark.append("(第");
		firstLogRemark.append(firstBorrow.getPeriods());
		firstLogRemark.append("期)");
		accountLogService.saveAccountLogByParams(accountVo, userId, effectiveMoney, firstLogRemark.toString(), HttpTookit.getRealIpAddr(request), "first_subscribe_cold",
				Constants.ACCOUNT_LOG_ID_TYPE_FIRST, firstBorrow.getId(), firstBorrow.getName());
	}

	/**
	 * <p>
	 * Description:用户投直通车时修改帐号信息<br />
	 * </p>
	 * 
	 * @author justin.xu
	 * @version 0.1 2014年7月16日
	...
	private Account packageAccount(AccountVo accountVo, BigDecimal effectiveMoney) throws Exception {
		...
		accountVo.setNoUseMoney(accountVo.getNoUseMoney().add(effectiveMoney));
	...
	private FirstBorrow modifyFirstBorrowWhenTender(FirstBorrowVo firstBorrowVo, Integer money) throws Exception {
		// 计算当前投标的有效金额
		Integer remaind = firstBorrowVo.getPlanAccount() - firstBorrowVo.getAccountYes();
		if (firstBorrowVo.getStatus() == Constants.FIRST_BORROW_STATUS_SUCCESS_APPROVE_PASS) {
			remaind = firstBorrowVo.getPlanAccount() - firstBorrowVo.getRealAccount();
		}
		...
		if (firstBorrowVo.getStatus() == Constants.FIRST_BORROW_STATUS_APPROVE_PASS) {
			...
			// 如果满标
			if (firstBorrowVo.getPlanAccount() - firstBorrowVo.getAccountYes() == 0) {
				firstBorrowVo.setStatus(Constants.FIRST_BORROW_STATUS_TO_SUCCESS_APPROVE);// 满标，待复审
				firstBorrowVo.setSuccessTime(new Date());
			}
		}
		// 上车
	...
	private String validateTenderFirstMoney(MemberVo memberVo, FirstBorrowOpenCnd firstBorrowOpenCnd, FirstBorrowVo firstBorrow, AccountVo account, BigDecimal effectiveMoney) throws Exception {
		...
		if (account.getUseMoney().compareTo(effectiveMoney) == -1) {
	...
	private String validateTenderFirstLogic(FirstBorrowOpenCnd firstBorrowOpenCnd, FirstBorrowVo firstBorrow) {
		...
		if (firstBorrow.getStatus() != Constants.FIRST_BORROW_STATUS_APPROVE_PASS && firstBorrow.getStatus() != Constants.FIRST_BORROW_STATUS_SUCCESS_APPROVE_PASS) {
			...
			if (firstBorrow.getStatus() == Constants.FIRST_BORROW_STATUS_APPROVE_PASS) {
				// 当已投金额大于等与计划金额时
				if (firstBorrow.getAccountYes() >= firstBorrow.getPlanAccount()) {
					msg.append("-直通车已满，无法继续开通！\n");
				}
			}
			// 如果状态在满标审核通过
			if (firstBorrow.getStatus() == Constants.FIRST_BORROW_STATUS_SUCCESS_APPROVE_PASS) {
				if (firstBorrow.getRealAccount() >= firstBorrow.getPlanAccount()) {
					msg.append("-直通车已满，无法继续开通！\n");
				}
			}
		}
	...
	private void packageFirstTenderRealAccountLog(Integer userId, HttpServletRequest request, AccountVo accountVo, BigDecimal effectiveMoney, FirstBorrow firstBorrow) throws Exception {
		// 开通账户日志-开通成功
		accountLogService.saveAccountLogByParams(accountVo, userId, effectiveMoney, "投标直通车：(第" + firstBorrow.getPeriods() + "期)" + "审核通过，扣除账户冻结金额。", HttpTookit.getRealIpAddr(request),
				"first_subscribe_success", Constants.ACCOUNT_LOG_ID_TYPE_FIRST, firstBorrow.getId(), firstBorrow.getName());
	}
	 ...
	 * @throws Exception void
	 */
	private FirstTenderReal packageFirstTenderReal(Integer userId, HttpServletRequest request, AccountVo accountVo, BigDecimal effectiveMoney, FirstBorrow firstBorrow) throws Exception {
		FirstTenderReal firstTenderReal = new FirstTenderReal();
		...
		firstTenderReal.setVersion(UUID.randomUUID().toString().replace("-", ""));
		// 再次上车
		firstTenderReal.setFirstTenderType(Constants.FIRST_BORROW_TENDER_TYPE_TWO);
		baseFirstTenderRealMapper.insertEntity(firstTenderReal);
	...
	public FirstBorrowVo getFirstBorrowForIndex(Date now) throws Exception {
		return firstBorrowMapper.getFirstBorrowForIndex(now);
	}

	@Override
	public FirstBorrowVo getAdvanceFirstBorrowForIndex(int firstBorrowId)
			throws Exception {
		return firstBorrowMapper.getAdvanceFirstBorrowForIndex(firstBorrowId);
	}
