...
public class HttpclientPost {
	...
	public static void main(String[] args) {
		...
		System.out.print(getHttpUrlPost("http://127.0.0.1:8080/dxjr_portal/api/tzj/login.html", postData));
	}
	 ...
	 * @date 2015年11月26日
	 */
	public static NameValuePair[] constructPostDate(Map<String, String> ruselt) {
		// 构造加密的请求参数
		Map<String, String> resultParamMap = ParseURLTool.buildEncryptionParameter(ruselt,ruselt.get("timestamp"));
		NameValuePair[] postData = { 
			new NameValuePair("from", resultParamMap.get("from")),																									// 服务端口名称
			new NameValuePair("service", resultParamMap.get("service")), 
			new NameValuePair("username", resultParamMap.get("username")), 
			new NameValuePair("usernamep", resultParamMap.get("usernamep")),																									// 服务端口名称
			new NameValuePair("registerAt", resultParamMap.get("registerAt")), 
			new NameValuePair("type", resultParamMap.get("type")), 
			new NameValuePair("timestamp", resultParamMap.get("timestamp")),
			new NameValuePair("sign", resultParamMap.get("sign"))			
		};
		return postData;
	}
}