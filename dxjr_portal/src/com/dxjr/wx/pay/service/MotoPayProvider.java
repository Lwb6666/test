...
public class MotoPayProvider {
	...
	public String process(String charset, String req) {
		...
		try {
			...
			httpPost = new HttpPost(OnlineContext.url);

			if (OnlineContext.url.indexOf("https") != -1) {
				KeyStore keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
				String path=MotoPayProvider.class.getResource("/").getPath() + "online/quick.keystore";
				System.out.println("key路径======================="+path);
