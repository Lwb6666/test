...
<div id="Bmain">
	...
	<div id="rz_main" style="height: 680px;">
		...
		<div class="rz_borrow">
			<table>
				...
				<tr>
					...
					<td><input value="${b.contractNo }" name="contractNo" class="rz_input" type="text" dataType="Limit" min="1" max="30" msg="借款单编号不能为空，且不能超过30个字符"/></td>
				</tr>
			</table>
		</div>
		<div class="rz_borrow">
		...
		<div class="rz_borrow">
			...
			<input <c:if test="${b.isMortgage==1 }">checked="checked"</c:if> id="isMortgage" name="isMortgage" type="radio" value="1" onclick="displaySet('mortgageDiv|mortgageTypeDiv','block');showRepayWays();"/>有抵押&nbsp;&nbsp; 
			<input <c:if test="${b.isMortgage==0 }">checked="checked"</c:if> name="isMortgage" type="radio" value="0" onclick="displaySet('mortgageDiv|mortgageTypeDiv','none');showRepayWays();"/>无抵押
			
			<c:if test="${borrowBusinessVo.userId==null || borrowBusinessVo.userId==''}">
		    	<input type="hidden" id="businessUserId" name="businessUserId" value="0"/>
		    </c:if>
		    <c:if test="${borrowBusinessVo.userId!=null && borrowBusinessVo.userId!=''}">
		    	<input type="hidden" id="businessUserId" name="businessUserId" value="${borrowBusinessVo.userId}"/>
		    </c:if>
		</div>
		<div class="rz_borrow1">
			<div class="rz_type">
				<div id="mortgageTypeDiv" class="rz_ltype">
					<span>抵押类型：</span> 
					<input <c:if test="${b.mortgageType==1 }">checked="checked"</c:if> name="mortgageType" type="radio" value="1" />房抵&nbsp;&nbsp;
					<input <c:if test="${b.mortgageType==2 }">checked="checked"</c:if> name="mortgageType" type="radio" value="2" />车抵&nbsp;&nbsp; 
					<input <c:if test="${b.mortgageType==3 }">checked="checked"</c:if> name="mortgageType" type="radio" value="3" />民品抵
				</div>
				<div class="rz_ltype">
					<span><input <c:if test="${b.isGuaranty==1 }">checked="checked"</c:if> id="isGuarantyCB" name="isGuarantyCB" type="checkbox" onclick="showRepayWays();" /></span>是否机构担保 
					<select id="guarantyOrganization" name="guarantyOrganization" msg="请选择担保机构">
						...
						<c:forEach items="${organizationOptions }" var="o">
							<option <c:if test="${b.guarantyOrganization==o.id }">selected="selected"</c:if> value="${o.id }">${o.name }</option>
						</c:forEach>
					</select>
				</div>
			</div>
				...
				<div class="rz_ltype1">
					<table border="0">
						<tr height="35">
							...
							<td><input value="${b.account }" id="account" name="account" class="rz_input1" type="text" dataType="Money|Compare" to="500" operator="GE" msg="借款金额格式有误|借款金额不能少于500元" maxlength="16"/></td>
						</tr>
						<tr>
							<td class="rz_font"><font class="needFlag">*</font>借款期限：</td>
							<td>
								<input id="timeLimit" name="timeLimit" value="${b.timeLimit }" class="rz_input1" require="true"   placeholder="借款期限必须为：1到99"     dataType="Integer|Range" min="1" max="99"  maxlength="2" msg="借款期限只能为整数|借款期限必须在1~99内" "/>
							</td>
						</tr>						
						<tr>
							<td class="rz_font">最高投标金额(元)：</td>
							<td><input value="${b.mostAccount }" id="mostAccount" name="mostAccount" class="rz_input1" type="text" require="false" dataType="Money|Range" msg="最高投标金额格式有误|最高投标金额不能小于最低投标金额，且不能大于借款金额" maxlength="16"/></td>
						</tr>
						<tr>
								...
								<div id="repaymentOptionsDiv">
									<select id="repaymentSel" name="style" class="rz_selected">
										<c:forEach items="${repaymentStyleOptions}" var="o">
											<option <c:if test="${b.style==o.name }">selected="selected"</c:if> value="${o.name}">${o.value}</option>
										</c:forEach>
									</select>
								</div>
							</td>
						...
						<tr>
							<td class="rz_font">投标密码：</td>
							<td>
								<input <c:if test="${b.bidPassword != '' && b.bidPassword!=null}">value="mimamima"</c:if> name="bidPassword" class="rz_input1" type="password" require="false" dataType="Character|Limit" min="6" max="12" msg="投标密码必须为字母或数字|投标密码长度必须在6~12位之间" maxlength="12" />
								<input value="${b.bidPassword }" name="oldBidPassword" type="hidden">
							</td>
						</tr>
					</table>
				</div>
				<div class="rz_ltype1">
					<table border="0">
						<tr>
							...
							<td><input value="${b.apr }" name="apr" value="6" class="rz_input1" type="text" dataType="Range|Money" min="6" max="24" msg="年化利率必须在6~24之间|年化利率格式有误"/></td>
						</tr>
						<tr>
							<td class="rz_font"><font class="needFlag">*</font>最低投标金额(元)：</td>
							<td><input value="${b.lowestAccount }" value="50" id="lowestAccount" name="lowestAccount" class="rz_input1" type="text" dataType="Money|Range" min="50" msg="最低投标金额格式有误|最低投标金额不能少于50元，且不能大于借款金额"/></td>
						</tr>
						<tr>
							<td class="rz_font"><font class="needFlag">*</font>有效期限(天)：</td>
							<td>
								<input value="${b.validTime }" value="3" name="validTime" class="rz_input1" type="text" dataType="Integer|Range" min="1" max="3" msg="有效期限只能为整数|有效期限必须在1~3天内" maxlength="1" />
							</td>
						</tr>
						<tr>
							...
							<td>
								<select name="pledgeType" id="pledgeType" class="rz_selected">
									<option <c:if test="${b.pledgeType==1 }">selected="selected"</c:if> value="1">新增</option>
									<option <c:if test="${b.pledgeType==2 }">selected="selected"</c:if> value="2">续贷</option>
									<option <c:if test="${b.pledgeType==3 }">selected="selected"</c:if> value="3">资产处理</option>						
								</select>
							</td>
						</tr>
					</table>
