	$(function(){
	//var jjrmodelidlist;  //用于存放从数据库取出的所有有待收的id，暂不用
	var jjrmodeltimelist; //用于存放从数据库取出的所有有待收的日期
	var jjrmodelztlist; //用于存放从数据库取出的所有有待收的状态，暂不用
...
function createSelectYear(){
	...
	for(var i=0;i<18;i++){
		yearSelect.options.add(new Option((i+2013)+"年",i+2013));
		if(currYear==i+2013){
...
//待收的三种状态jjrmodeltimelist，默认为无待收，1为有待收，2为已待收
function getjjrszModelByYear(year){
	getjjrszModelByYear(year);
}
function setA(){
	...
	for(var i=0;i<arr.length;i++){
		...
		var datearr=date.split("-");
			...
			for(var n=0;n<jjrmodeltimelist.length;n++){
				if(formatByDate(jjrmodeltimelist[n])==formatByDate(date)){
					if(jjrmodelztlist[n] == 1){ //待收样式
						var span=newElement('span');
						span.setAttribute("class","daishou");
						arr[i].style.background="#fff0f0";
						arr[i].appendChild(span);
					}else if(jjrmodelztlist[n] == 2){ //2已收样式
						var span=newElement('span');
						span.setAttribute("class","daishou-past");
						arr[i].style.background="#f5f5f5";
						arr[i].appendChild(span);
					}else if(jjrmodelztlist[n] == 3){ 
