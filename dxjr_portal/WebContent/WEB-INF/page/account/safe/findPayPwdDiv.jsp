...
<form id="findPwdForm" name="findPwdForm" action="">
	<div class="cont-word">
    	<div class="title"><h4>找回交易密码</h4><a href="javascript:void(0);" onclick="closeFindPayPwd();" class="icon icon-close fr"></a></div>
        <div class="form-info-layer">
            <div class="form-col2">
                <label for="" class="colleft02 form-lable">用户名</label>
                <input type="text"  style="width:198px" class="colright form-inpyt-sm" placeholder="请输入用户名"
                 name="username"  dataType="Limit" min="1" max="20" maxlength="20">
            </div>
            <div class="form-col2">
                <label for="" class="colleft02 form-lable">真实姓名</label>
                <input type="text" style="width:198px" class="colright form-inpyt-sm" placeholder="请输入真实姓名"
                name="realname"  dataType="Limit" min="1" max="20" maxlength="20">
            </div>
            <div class="form-col2">
                <label for="" class="colleft02 form-lable">身份证号</label>
                <input type="text" style="width:198px" class="colright form-inpyt-sm" placeholder="请输入身份证号"
                name="idCard" dataType="Limit" min="1" max="20" maxlength="20">
            </div>
            <div class="form-col2">
                <label for="" class="colleft02 form-lable">验证码</label>
                <input type="text"  style="width:120px" class="colright form-inpyt-sm"  placeholder="验证码"
                id="checkcode" name="checkcode" dataType="Require" maxlength="4" >
            ...
            <div class="form-col2">
            <button type="button" class="remove" onclick="closeFindPayPwd();">取消</button><button type="button" class="enter" id="submitBtn" class="enter" onclick="save();" >确定</button>
            </div>
         </div>
    </div> 
</form>
	...
	function save(){
		...
		layer.confirm('确定提交吗？', function(){
			$("#findPwdForm").ajaxSubmit({
				url : '${basePath}/account/safe/findPayPwd.html',
				async:false,
				type : 'post',
				dataType : 'json',
				beforeSend:function(){
				...
				success : function(data) {
					...
					if(data.code=='0'){
						...
						if($('input[name="findType"]:checked').val()=='mobile'){
                    		layer.alert("交易密码重置成功，已发送至您的认证手机，为了您的账户安全请尽快修改！！" , 1, "交易密码重置",function(){
    							window.location.href="${basePath}/AccountSafetyCentre/safetyIndex.html";
    						});
                    	}
                    	if($('input[name="findType"]:checked').val()=='email'){
                    		layer.alert("交易密码重置成功，已发送至您的认证邮箱，为了您的账户安全请尽快修改！！" , 1, "交易密码重置",function(){
    							window.location.href="${basePath}/AccountSafetyCentre/safetyIndex.html";
    						});
                    	}
					}
