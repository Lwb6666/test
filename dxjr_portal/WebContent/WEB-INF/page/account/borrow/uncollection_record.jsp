            ...
            <div class="tz-dqb2 clearfix">
                <div class="col clearfix"><span class="fl gary2">是否存管：</span>
                  <div class="btn-box-bg">
                        <div class="type">
                            <a <c:if test="${custody== '' || custody == null }"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','nochange','all');">全部</a>
                        </div>
                        <div class="type">
                            <a <c:if test="${custody == '0' }"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','nochange','0');">非存管</a>
                        </div>
                        <div class="type">
                            <a <c:if test="${custody == '1' }"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','nochange','1');">存管</a>
                        </div>
               ...
               <div class="col clearfix">
                        ...
                        <div class="btn-box-bg">
                            <div class="term"><a href="javascript:queryrecord('unCollection_record','nochange');" class="active">待收明细</a></div>
                            <div class="term"><a href="javascript:forqueryrecord('receivedCollection_record');" >已收明细</a></div>
                        </div>
                </div>
                ...
                <div class="col clearfix">
                        ...
                        <div class="btn-box-bg">
                            <div class="term"><a <c:if test="${timeScope == 'all' || timeScope == '' || timeScope== null}"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','all');">全部</a></div>
                            <div class="term">
                            ...
                            <div class="term"><a href="javascript:queryrecord('unCollection_record');" class="active" >查询</a></div>
                            
                            <input type="hidden" id="timeScope" name="timeScope" value="${timeScope }"/>
                            
                            <div class="term"><a <c:if test="${timeScope == 'month'}"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','month');" >近1月</a></div>
                            <div class="term"><a <c:if test="${timeScope == 'threemonth'}"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','threemonth');">近3月</a></div>
                            <div class="term"><a <c:if test="${timeScope == 'sixmonth'}"> class="active"</c:if> href="javascript:queryrecord('unCollection_record','sixmonth');">近6月</a></div>
            ...
            <div class="col clearfix">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0"  class="table center tbtr">
                  ...
                  <c:forEach items="${CollectionRecordList}" var="collectionRecord" varStatus="sta" step="1">
	                  <tr>
	                    <td><c:set var="borrow" value="${borrowMap[collectionRecord.tenderId]}"></c:set>
	                    ...
	                    <td align="left"><c:if test="${collectionRecord.iscustody == 1 }"><i class="icon icon-zs" title="银行存管"></i></c:if>
	                    	...
	                    	<a title='${collectionRecord.name }' href="${path }/toubiao/${borrow.id}.html" target="_blank">
								${fn:substring(collectionRecord.name,0,10)}
						...
						<c:choose>
						   <c:when test="${collectionRecord.ishowfloat==1 && collectionRecord.iscustody != 1}">
	 							<c:if test="${collectionRecord.parentId==null}">
						           <td><span>${collectionRecord.apr }%</span> <span style="position:absolute; color:red; font-size:12px">+${collectionRecord.yearApr}%</span></td>
						      </c:if>						  
						   </c:when>
						   <c:otherwise>
...
<c:if test="${page.result !=null && page.totalCount > 0 }">
	...
	function exportCollectionRecord(url){
		url = '${basePath}/account/InvestManager/exportCollectionRecord/0/0.html';
	...
	*/
	...
	function findPage(pageNo){
		window.parent.turnCollectionPageParent('unCollection_record',pageNo,0,0);
	}
	/**
	 * hujianpan
	 * 切换统计数据,即已收和待收页签的切换
