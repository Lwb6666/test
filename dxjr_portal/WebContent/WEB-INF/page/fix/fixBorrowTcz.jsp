	  ...
	  <c:forEach items="${page.result}" var="fixBorrowJoinDetail" varStatus="sta">
		 <tr>
		    <td align="left" class="tdtitl07">
		    	<a href="${path }/dingqibao/detailTcz/${fixBorrowJoinDetail.fixBorrowId }.html" class="blue">
		    ...
		    <td align="right" <c:if test="${fixBorrowJoinDetail.floatApr > 0}">title="加息利息：<fmt:formatNumber value="${fixBorrowJoinDetail.floatInterest}" pattern="#,##0.00" /> "</c:if>>${fixBorrowJoinDetail.hasReturnMoneyStr } </td>
		    <td class="blue">${fixBorrowJoinDetail.cnt }</td>
		    <td class="blue">
		    <c:if test="${fixBorrowJoinDetail.exitType ==1}">到期退出</c:if>
		    <c:if test="${fixBorrowJoinDetail.exitType ==2}">提前退出</c:if>
		    </td>
